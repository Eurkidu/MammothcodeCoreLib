...
Mc.Pc.Pop.Dialog = function (options) {
    ...
    if (settings.otherClose !== "") {
        $popObj.on("click", settings.otherClose, function () {
            //回调函数为空->关闭||回调函数不为空判断是否为函数->true->执行函数->函数返回值为false不关闭弹出层,否则关闭弹出层
            if (settings.otherClose == null || typeof (settings.otherClose) === "function" && settings.otherClose() !== false) {
                popClose();
            }
        });
    }
...
Mc.Pc.Calendar.SignCal = {
    ...
    iniCal: function (options) {
        ...
        //签到按钮事件
        $signInObj.on("click", "#sign_in_btn", function () {
    ...
    drawCal: function (options) {
        ...
        //日历数据主体
        htmls.push(Mc.Pc.Calendar.SignCal.getCalDataHtml(settings.iYear, settings.iMonth, settings.signList));
        htmls.push("</div>");
        htmls.push("</div>");
        return htmls.join('');
    },
    ...
    updateCal: function (options) {
        ...
        //添加新数据
        $signCal.html(Mc.Pc.Calendar.SignCal.getCalDataHtml(settings.iYear, settings.iMonth, settings.signList));
        //回调函数为空||回调函数不为空判断是否为函数->true->执行函数
     ...
     */
    getCalDataHtml: function (iYear, iMonth, signList) {
        //判断当天是否已签到
        ...
        htmls.push("<table>");
        htmls.push("<tr>");
        htmls.push("<th class='col-1'>" + myMonth[0][0] + "</th>");
        htmls.push("<th class='col-2'>" + myMonth[0][1] + "</th>");
        htmls.push("<th class='col-3'>" + myMonth[0][2] + "</th>");
        htmls.push("<th class='col-4'>" + myMonth[0][3] + "</th>");
        htmls.push("<th class='col-5'>" + myMonth[0][4] + "</th>");
        htmls.push("<th class='col-6'>" + myMonth[0][5] + "</th>");
        htmls.push("<th class='col-7'>" + myMonth[0][6] + "</th>");
        htmls.push("</tr>");
        var d, w;
        for (w = 1; w < 7; w++) {
            htmls.push("<tr>");
            for (d = 1; d < 8; d++) {
                ...
                if (signed) {
                    htmls.push("<td mc-cal-value='" + dayDate + "' class='col-" + d + " on'>" + dayDate + "</td>");
                } else {
                    if (Mc.Util.isNull(dayDate)) {
                        htmls.push("<td mc-cal-value='" + dayDate + "' class='col-" + d + " other-month'>" + dayDate + "</td>");
                    } else {
                        htmls.push("<td mc-cal-value='" + dayDate + "' class='col-" + d + "'>" + dayDate + "</td>");
                    }
                }
            }
            htmls.push("</tr>");
...
Mc.Util.Date.getCalendar = function (options) {
    ...
    var settings = $.extend({
        iYear: current.getFullYear(), //年 | 默认 当前年份
        iMonth: current.getMonth() + 1, //月 | 默认 当前月份
        calHead: ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"] //日历表头 | 默认 星期日~星期一
    }, options);
