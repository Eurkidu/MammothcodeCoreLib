...
Mc.Util.EffectChange = function(options) {
    var settings = $.extend({
        target: "document", //特效编号父级jQ选择器 | 默认 "document"
        event: "click", //触发事件 默认click
        effect: "SH", //特效形式 默认SH-show/hide | FIO-fadeIn/fadeOut | SDU-slideDown/slideUp
        onlyActive: false //同一时间只能有一个激活 默认false
    }, options);
    var effectWay = function() {
    ...
    var $targetF = $(settings.target);
    $targetF.on(settings.event, "[mc-efc-btn]", function () {
        var $this = $(this); //当前对象
        var $targetData = $this.attr("mc-efc-btn"); //当前对象目标数据
        var $target = $targetF.find("[mc-efc-page=" + $targetData + "]"); //当前对象目标
        if (settings.onlyActive) {
            var $lastBtn = $targetF.find("[mc-efc-btn].mc-active"); //上一次对象
            var $lastPage = $targetF.find("[mc-efc-page].mc-active"); //上一次对象目标
            //关闭上一次对象
            $lastBtn.removeClass("mc-active active");
            effectEnd($lastPage);
        }
        //打开当前对象
        $this.addClass("mc-active active");
        effectStart($target);
    });
}
