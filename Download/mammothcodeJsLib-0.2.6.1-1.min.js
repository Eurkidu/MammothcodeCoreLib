...
Mc.App.Pop.Tip = function (options) {
    ...
    //操作提示显示
    $popObj.fadeIn(args.fadeSpeed, function () {
        //操作提示延时自动消失
        setTimeout(function () {
            $popObj.fadeOut(args.fadeSpeed, function () {
                $popObj.remove();
            });
        }, args.tipLive);
    });
...
Mc.Pc.Pop.Dialog = function (options) {
    ...
    var popHtml = function typeTipHtmlCreate() {
        ...
        html += "<div class='pop-wrap " + args.wrapClass + "'>";
        //关闭按钮
        if (args.closeBtn) html += "<div id='close_" + popId + "' class='pop-close " + args.closeBtnClass + "'></div>";
        //Pop html
        html += args.popHtml;
        //弹出层btn构造
        if (args.leftBtn) html += "<div id='dialog_L_" + popId + "' class='btn " + args.leftBtnClass + "'>" + args.leftBtnContent + "</div>";
        if (args.rightBtn) html += "<div id='dialog_R_" + popId + "' class='btn " + args.rightBtnClass + "'>" + args.rightBtnContent + "</div>";
        //弹出层bg构造
        if (args.hasBg) html += "</div><div id='bg_" + popId + "' class='pop-bg'></div>";
        html += "</div>";
        return html;
    };
    //清除之前错误未显示的弹窗
    ...
    if (args.iniCallback == null || typeof (args.iniCallback) === "function" && args.iniCallback() !== false) {
        //弹窗显示
        $popObj.fadeIn(args.fadeSpeed);
    }
    //弹窗关闭
    function popClose() {
        //回调函数为空->关闭||回调函数不为空判断是否为函数->true->执行函数->函数返回值为false不关闭弹出层,否则关闭弹出层
        if (args.fadeOutCallback == null || typeof (args.fadeOutCallback) === "function" && args.fadeOutCallback() !== false) {
            $popObj.fadeOut(args.fadeSpeed, function () {
                $popObj.remove();
            });
        }
    ...
    if (args.leftBtn) {
        ...
        $popObj.on("click", "#dialog_L_" + popId, function () {
            //回调函数为空->关闭||回调函数不为空判断是否为函数->true->执行函数->函数返回值为false不关闭弹出层,否则关闭弹出层
            if (args.leftBtnCallback == null || typeof (args.leftBtnCallback) === "function" && args.leftBtnCallback() !== false) {
                popClose();
            }
        });
    }
    if (args.rightBtn) {
        ...
        $popObj.on("click", "#dialog_R_" + popId, function () {
            //回调函数为空->关闭||回调函数不为空判断是否为函数->true->执行函数->函数返回值为false不关闭弹出层,否则关闭弹出层
            if (args.rightBtnCallback == null || typeof (args.rightBtnCallback) === "function" && args.rightBtnCallback() !== false) {
                popClose();
            }
        });
    }
    ...
    if (args.bgClose) {
        $popObj.on("click", "#bg_" + popId, function () {
            //回调函数为空->关闭||回调函数不为空判断是否为函数->true->执行函数->函数返回值为false不关闭弹出层,否则关闭弹出层
            if (args.bgCloseCallback == null || typeof (args.bgCloseCallback) === "function" && args.bgCloseCallback() !== false) {
                popClose();
            }
        });
    }
    //其他关闭事件
    if (args.otherClose !== "") {
        $popObj.on("click", args.otherClose, function () {
            //回调函数为空->关闭||回调函数不为空判断是否为函数->true->执行函数->函数返回值为false不关闭弹出层,否则关闭弹出层
            if (args.otherCloseCallback == null || typeof (args.otherCloseCallback) === "function" && args.otherCloseCallback() !== false) {
                popClose();
            }
        });
    }
...
Mc.Pc.Pop.DialogForm = function (options) {
    ...
    var popHtml = function typeTipHtmlCreate() {
        ...
        html += "<div id='Dialog" + popId + "' class='pop-position-wrap' style='display:none;'>";
        html += "<div class='pop-wrap " + args.wrapClass + "'>";
        html += "<form action='" + args.formAction + "' method='" + args.formMethod + "' target=\"_blank\">";
        //Pop html
        html += args.popHtml;
        //弹出层btn构造
        if (args.leftBtn) html += "<input type='submit' id='dialog_L_" + popId + "' class='btn " + args.leftBtnClass + "' value='" + args.leftBtnContent + "'>";
        if (args.rightBtn) html += "<input type='submit' id='dialog_R_" + popId + "' class='btn " + args.rightBtnClass + "' value='" + args.rightBtnContent + "'>";
        //弹出层bg构造
        if (args.hasBg) html += "</form></div><div id='bg_" + popId + "' class='pop-bg'></div>";
        html += "</div>";
        return html;
    };
    //清除之前错误未显示的弹窗
...
Mc.Pc.Pop.Loading = function (options) {
    ...
    var popHtml = function typeTipHtmlCreate() {
        ...
        html += "<div class='pop-wrap " + args.wrapClass + "'>";
        html += "<div class='" + args.loadingClass + "'>";
        //loading txt
        html += args.loadingTxt;
        //弹出层bg构造
        if (args.hasBg) html += "</div></div><div id='bg_" + popId + "' class='pop-bg'></div>";
        html += "</div>";
        return html;
    };
    //添加弹出层html到页面上
...
Mc.Pc.Calendar.SignCal = {
    ...
    iniCal: function (options) {
        ...
        var signHtml = Mc.Pc.Calendar.SignCal.drawCal({
            ...
            iMonth: args.iMonth,
            iDay: args.iDay,
            signList: args.signList
        });
        //添加HTML到页面
        $(args.target).html(signHtml);
        ...
        if (args.iniCallback == null || typeof (args.iniCallback) === "function" && args.iniCallback() !== false) {
            ...
            Mc.Util.selectMenu("#sign_year_select", function (_this, val) {
                ...
                var dif = parseInt(parseInt((dateTime.year - args.iYear) * 12) + parseInt((dateTime.month - args.iMonth)));
                if (dif > 0) {
                ...
                //年份下拉选择框回调函数
                args.yearSelectCallBack(dateTime);
            });
            Mc.Util.selectMenu("#sign_month_select", function (_this, val) {
                ...
                //月份下拉选择框回调函数
                args.monthSelectCallBack(dateTime);
            });
            //日历显示
            $signInObj.show();
        }
        ...
        function signIn() {
            ...
            if (args.signInBtnCallBack == null || typeof (args.signInBtnCallBack) !== "function") {
                //签到失败
                //console.error(errorMsg); //输出错误信息
                //回调函数为空||回调函数不为空判断是否为函数->true->执行函数
                if (args.signInFailCallBack == null || typeof (args.signInFailCallBack) === "function" && args.signInFailCallBack());
            } else {
                var data = args.signInBtnCallBack(); //获得回调函数返回值
                ...
                if (data.state == true) {
                    //签到成功
                    //回调函数为空||回调函数不为空判断是否为函数->true->执行函数
                    if (args.signInSuccessCallBack == null || typeof (args.signInSuccessCallBack) === "function" && args.signInSuccessCallBack());
                } else {
                    //签到失败
                    //回调函数为空||回调函数不为空判断是否为函数->true->执行函数
                    if (args.signInFailCallBack == null || typeof (args.signInFailCallBack) === "function" && args.signInFailCallBack());
        ...
        //其他签到按钮事件绑定
        if (!Mc.Util.isNull(args.otherSignInBtn)) {
            $(document).on("click", args.otherSignInBtn, function () {
                signIn();
            });
        }
    },
    ...
    drawCal: function (options) {
        ...
        //年份选择
        htmls.push("<div class='select-box' id='sign_year_select'><input type='hidden' value='" + args.iYear + "'><div>" + args.iYear + "年</div><ul style='display:none;'>");
        for (var i = 2015; i < 2050; i++)
            htmls.push("<li mc-select-value='" + i + "'>" + i + "年</li>");
        htmls.push("</ul></div>");
        //月份选择
        htmls.push("<div class='select-box' id='sign_month_select'><input type='hidden' value='" + args.iMonth + "'><div>" + args.iMonth + "月</div><ul style='display:none;'>");
        for (var j = 1; j < 13; j++)
            htmls.push("<li mc-select-value='" + j + "'>" + j + "月</li>");
        htmls.push("</ul></div>");
        //标题
        //htmls.push("<div class='calendar_month_span'>" + args.iYear + "年" + args.iMonth + "月</div>");
        ...
        //日历数据主体
        htmls.push(Mc.Pc.Calendar.SignCal.getCalDataHtml(args.iYear, args.iMonth, args.iDay, args.signList));
        htmls.push("</div>");
        htmls.push("</div>");
        return htmls.join('');
    },
    ...
    updateCal: function (options) {
        ...
        //添加新数据
        $signCal.html(Mc.Pc.Calendar.SignCal.getCalDataHtml(args.iYear, args.iMonth, args.iDay, args.signList));
        //回调函数为空||回调函数不为空判断是否为函数->true->执行函数
        if (args.callBack == null || typeof (args.callBack) === "function" && args.callBack());
    },
...
Mc.Util.selectBox = function (options) {
    ...
    function selectBoxIni() {
        ...
        //分组
        $target.attr("mc-group", args.group);
    }
    $(function () {
        selectBoxIni();
    });
    ...
    $target.on("click", function (ev) {
        if ($targetLi.length) {
            ...
            if ($this.hasClass("mc-active")) {
                ...
                //关闭同组其他下拉框
                $("[mc-group=" + args.group + "]").removeClass("mc-active").find("ul").hide();
    ...
    $target.on("click", 'li', function (ev) {
        ...
        if (args.callback != null) args.callback(this, mcSelectValue);
        $targetUl.hide();
...
Mc.Util.EffectChange = function (options) {
    ...
    function effectChageIni() {
        $(args.target + " [mc-efc-page]").each(function (i, val) {
            var $active = $(val);
    ...
    function effectStart(efcTarget) {
        ...
        switch (effectType) {
            case 0:
                ...
                break;
            case 1:
                efcTarget.fadeIn(args.effectSpeed);
                break;
            case 2:
                efcTarget.slideDown(args.effectSpeed);
                break;
        }
    }
    //结束特效
    function effectEnd(efcTarget) {
        ...
        switch (effectType) {
            case 0:
                ...
                break;
            case 1:
                efcTarget.fadeOut(args.effectSpeed);
                break;
            case 2:
                efcTarget.slideUp(args.effectSpeed);
                break;
        }
    }
    var $targetF = $(args.target); //获取绑定对象
    $targetF.on(args.event, "[mc-efc-btn]", function () {
        var $this = $(this); //当前对象
        ...
        var condition1 = !args.lessOneActive; //条件1 - 可以全处于冻结状态
...
Mc.Util.iniFixCountDown = {
    ...
    start: function (options) {
        ...
        timeFormatTxt = args.timeFormat.split("%d");
        ...
        if (Mc.Util.iniFixCountDown.timeCountDownState) {
            ...
            if (args.startCallback != null) args.startCallback(); //开始节点回调函数
            Mc.Util.iniFixCountDown.interValObj = window.setInterval(function () {
                if (args.fixTime == 0) {
                    window.clearInterval(Mc.Util.iniFixCountDown.interValObj);
                    $(args.target).html(args.defaultTxt);
                    if (args.endCallback != null) args.endCallback(); //结束节点回调函数
                    Mc.Util.iniFixCountDown.timeCountDownState = true;
                } else {
                    args.fixTime--;
                    $(args.target).html(timeFormatTxt[0] + parseInt(args.fixTime) + timeFormatTxt[1]);
                }
            }, 1000);
        }
    },
...
Mc.Util.Date.getCalendar = function (options) {
    ...
    var dCalDate = new Date(args.iYear, args.iMonth - 1, 1);
    var iDayOfFirst = dCalDate.getDay(); //获得当月第一天
    var iDaysInMonth = Mc.Util.Date.getDaysInmonth(args.iYear, args.iMonth); //获得当月总天数
    var iVarDate = 1;
    var d, w;
    //表头初始化
    for (var i = 0; i < 7; i++) {
        aMonth[0][i] = args.calHead[i];
    }
...
/**
 * [Checkphonecode 正则表达式验证QQ号是否正确]
 * @param  {varchar} qqCode
 * @return {Boolean}
 */
Mc.Util.Check.checkPhoneCode = function (qqCode) {
    var reg;
    if (Mc.Util.String.isOnlyOne(qqCode)) {
        reg = /^[1-9][0-9]{5,10}/;
        return reg.test(qqCode);
    }
    return false;
};
...
Mc.Util.Uri.goNextUrl = function (targetUrl) {
    ...
    if (oldUrl != null) {
        ...
        //解码
        goUrl = targetUrl + "oldUrl=" + Mc.Util.Uri.urlEncode(oldUrl);
...
function Dialog(options) {
    ...
    function typeDefaultHtmlCreate() {
        ...
        HTML += "<div class='dialog-wrap ds" + args.style + "' style='width:" + args.width + ";max-height:" + args.max_height + ";'>";
        ...
        if (args.hasHead) {
            ...
            if (isEmptyString(args.headHTML)) {
                HTML += args.headHTML;
            } else {
                HTML += "<div class='dialog-head ds" + args.style + "'>" + args.head_txt + "</div>";
            }
        }
        HTML += "</div>";
        ...
        if (isEmptyString(args.bottomHTML)) {
            HTML += args.bottomHTML;
        } else {
            //判断按钮显示
            if (args.confirm_btn || args.cancel_btn) {
                HTML += "<div class='dialog-btn-wrap ds" + args.style + "'>";
                if (args.confirm_btn)
                    HTML += "<div id='dialog_L_" + dialogId + "' class='dialog-btn ds" + args.style + "'>" + args.confirm_txt + "</div>";
                if (args.cancel_btn)
                    HTML += "<div id='dialog_R_" + dialogId + "' class='dialog-btn ds" + args.style + "'>" + args.cancle_txt + "</div>";
                HTML += "</div>";
            }
        }
        HTML += "</div>";
        ...
        //dialog-bg构造
        if (args.hasBg) HTML += "<div id='bg_" + dialogId + "' class='dialog-bg ds" + args.style + "'></div>";
    ...
    if (args.type == "tip") {
        ...
        //操作提示显示
        $("#tip" + dialogId).fadeIn(args.fadeSpeed, function () {
            //操作提示延时自动消失
            setTimeout(function () {
        ...
        $("body").before(dialogHtml);
        //载入中弹出层显示
        $("#loading" + dialogId).fadeIn(args.fadeSpeed);
    } else {
        $("body").before(dialogHtml);
        //弹出层显示
        $("#" + dialogId).fadeIn(args.fadeSpeed, function () {
            //设置dialog最大高度
            setDialogMaxHeight();
            //弹出层初始化回调函数
            if (args.callback_ini != null) args.callback_ini();
            //滚动条更新(CANCEL)
            //if (args.scroll) {
            //    $(".dialog-body").mCustomScrollbar("update");
            //}
    ...
    if (args.type == "tip") {
        ...
        //其他关闭按钮
        if (args.otherClose) {
            $(args.otherClose).live('click', function () {
                //左侧按钮操作
                if (args.callback_oc != null) args.callback_oc();
                //隐藏并删除弹出层
                delDialog(dialogId);
            });
        }
        //底部按钮
        if ((args.confirm_btn || args.cancel_btn) && args.type != "void") {
            if (args.confirm_btn) {
                $(args.confirm_id + dialogId).bind('click', function () {
                    //左侧按钮操作
                    if (args.callback_L != null) args.callback_L();
                    //隐藏并删除弹出层
                    delDialog(dialogId);
                });
            }
            if (args.cancel_btn) {
                $(args.cancel_id + dialogId).bind('click', function () {
                    //右侧按钮操作
                    if (args.callback_R != null) args.callback_R();
                    //隐藏并删除弹出层
                    delDialog(dialogId);
                });
            }
        ...
        if (args.bgClose) {
            $("#bg_" + dialogId).bind('click', function () {
                //关闭弹出框后操作
                if (args.callback_bc != null) args.callback_bc();
                //隐藏并删除弹出层
                delDialog(dialogId);
            });
        }
    ...
    function delDialog(dialogNum) {
        $("#" + dialogNum).fadeOut(args.fadeSpeed, function () {
            //弹出层消失回调函数
            if (args.callback_close != null) args.callback_close();
            $("#" + dialogNum).remove();
        });
    }
    ...
    function setDialogMaxHeight() {
        ...
        dialogBodyMaxHeight = $(window).height() * (parseInt(args.max_height) / 100) - dialogHeadH - dialogBtnH;
    ...
    //        //mouseWheelPixels: args.scroll_mouseWheelPixels
