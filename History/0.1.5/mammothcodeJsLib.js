...
Mc.Pc.Pop = {
    Dialog: function (options) {
        var settings = $.extend({
            ...
            leftBtn: true,
            leftBtnClass: "",
            leftBtnContent: "确认", //左侧按钮文字,默认确定
            leftBtnCallback: null, //左侧按钮回调函数
            rightBtn: true,
            rightBtnClass: "",
            rightBtnContent: "取消", //右侧按钮文字,默认取消
            rightBtnCallback: null, //右侧按钮回调函数
            hasBg: true, //是否有bg,默认true
            fadeSpeed: 200, //默认淡入淡出时间200毫秒
            tipLive: 2000 //操作提示层生存时间(单位毫秒)
        }, options);
        //把当前时间(从1970.1.1开始的毫秒数)给弹出层作为ID标识
        ...
        var popHtml = function typeTipHtmlCreate() {
            ...
            if (settings.leftBtn) html += "<div id='dialog_L_" + popId + "' class='btn able " + settings.leftBtnClass + "'>" + settings.leftBtnContent + "</div>";
        ...
        $popObj.on("click", "#dialog_L_" + popId, function () {
            if (settings.leftBtnCallback != null) settings.leftBtnCallback();
            $popObj.fadeOut(settings.fadeSpeed, function () {
                $popObj.remove();
            });
        });
        ...
        $popObj.on("click", "#dialog_R_" + popId, function () {
            if (settings.rightBtnCallback != null) settings.rightBtnCallback();
            $popObj.fadeOut(settings.fadeSpeed, function () {
                $popObj.remove();
            });
        });
...
//手风琴菜单
//navList 手风琴菜单对象的id或class选择器
//oneSildeDown 是否只有一个能展开 true 一个 false 多个
