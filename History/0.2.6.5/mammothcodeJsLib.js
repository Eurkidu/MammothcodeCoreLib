        ...
        init: function (obj, args) {
            return (function () {
                ...
                if (args.isLoadFirst) { //如果要自动加载第一页
                    ...
                    ms.msData.pageCount = args.pageCount; //默认已加载完成第一页,总页数使用参数值
                }
                ms.fillHtml(obj, { "current": ms.msData.pageIndex + 1, "pageCount": ms.msData.pageCount });
...
/**
 * [工具函数-自定义下拉选择框v2]
 * [last alter 2015-10-10 13:47:38 by zero]
...
Mc.Util.selectBox = function (options) {
    ...
    //设置下拉框当前值
    function setValue(txt, mcSelectValue, triggerCallback) {
        $targetInput.val(mcSelectValue);
        $targetDiv.text(txt);
        if (triggerCallback) {
            settings.callback != null && settings.callback(this, mcSelectValue); //触发回调函数
        }
    }
    //更新下拉候选项
    function update() {
